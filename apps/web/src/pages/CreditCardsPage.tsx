...
const CreditCardsPage = () => {
  ...
  const [form, setForm] = useState({ name: '', issuer: '', last4: '', creditLimit: '', statementClosingDay: '25', paymentDueDay: '10', color: '#1A1F36' });
  ...
  const create = useMutation({
    ...
    onSuccess: (response) => { const created = response.data; queryClient.setQueryData<CreditCard[]>(['credit-cards'], (old = []) => old.some((card) => card._id === created._id) ? old : [...old, created]); queryClient.invalidateQueries({ queryKey: ['credit-cards'] }); setShowForm(false); setForm({ name: '', issuer: '', last4: '', creditLimit: '', statementClosingDay: '25', paymentDueDay: '10', color: '#1A1F36' }); },
  });
  const [payment, setPayment] = useState('');
  const pay = useMutation({
    mutationFn: async () => api.post(`/credit-cards/${selected._id}/payments`, { statementMonth: current.getMonth() + 1, statementYear: current.getFullYear(), amount: Number(payment), paidAt: new Date().toISOString() }),
