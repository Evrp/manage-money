...
const BudgetsPage = () => {
  ...
  const totals = useMemo(() => {
    if (!Array.isArray(budgets))
      return { actualSpent: 0, budgetedSpent: 0, limit: 0, percent: 0 };

    const actualSpent = budgets.reduce((acc, b) => acc + b.spentAmount, 0);
    const budgetedItems = budgets.filter((b) => b.limitAmount > 0);
    const budgetedSpent = budgetedItems.reduce(
      (acc, b) => acc + b.spentAmount,
      0,
    );
  ...
  const budgetedBudgets = Array.isArray(budgets)
    ? budgets.filter((b) => b.limitAmount > 0)
    : [];
  const unbudgetedBudgets = Array.isArray(budgets)
    ? budgets.filter((b) => b.limitAmount === 0)
    : [];
    ...
    <Layout>
      <div className="flex flex-col gap-6">
        <header className="flex justify-between items-center px-1">
          <div>
            <span className="eyebrow">SPEND WITH INTENTION</span>
            <h1 className="text-2xl font-black">วางแผนให้เงินของคุณ</h1>
            <p className="text-sm text-gray-500 mt-2">
              จัดสรรงบประมาณ แล้วใช้จ่ายอย่างมั่นใจ
            </p>
          </div>
          <div className="flex gap-2">
            <button
              onClick={() => setShowCreateModal(true)}
              aria-label="เพิ่มหมวดหมู่งบประมาณ"
            ...
            <div className="relative">
              <button
                ...
                className="bg-surface p-2.5 rounded-2xl border border-gray-100 shadow-sm text-gray-400 hover:text-indigo-600 hover:border-indigo-100 transition-all"
              >
                <Calendar size={20} />
              </button>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-surface p-6 rounded-[2.5rem] border border-gray-100 shadow-sm relative overflow-hidden group">
            <div className="relative z-10">
              <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-2">
                รายจ่ายจริงทั้งหมดในเดือนนี้
              </p>
              <h3 className="text-3xl font-black text-gray-800">
                ฿{totals.actualSpent.toLocaleString()}
              </h3>
            </div>
          ...
          <div className="bg-indigo-600 p-6 rounded-[2.5rem] text-white shadow-xl shadow-indigo-100 relative overflow-hidden">
            <div className="relative z-10">
              <p className="text-indigo-100 text-[10px] font-bold uppercase tracking-widest mb-2">
                การใช้จ่ายเปรียบเทียบกับงบ
              </p>
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-4xl font-black font-sans">
                  ฿{totals.budgetedSpent.toLocaleString()}
                </span>
                {totals.limit > 0 && (
                  <span className="text-indigo-200 text-lg">
                    / ฿{totals.limit.toLocaleString()}
                  </span>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                {totals.limit > 0 && (
                  <div className="h-3 bg-white/20 rounded-full overflow-hidden p-0.5">
                    <div
                      className={`h-full rounded-full transition-all duration-1000 ${totals.percent > 90 ? "bg-red-400" : "bg-surface"}`}
            ...
            <Target
              className="absolute -right-4 -bottom-4 text-indigo-500/30"
              size={120}
            />
          </div>
        </section>
          ...
          {isLoading ? (
            ...
            budgetedBudgets.map((b) => {
              ...
              return (
                <div
                  key={b._id}
                  className="bg-surface p-6 rounded-[2rem] border border-gray-100 shadow-sm hover:translate-y-[-2px] transition-all"
                >
                  <div className="flex justify-between items-start mb-5">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-3 h-3 rounded-full ${!hasLimit ? "bg-gray-300" : isDanger ? "bg-red-500" : "bg-indigo-300"}`}
                      ></div>
                    ...
                    <button
                      aria-label={`แก้ไขงบ ${b.categoryId?.name || "อื่นๆ"}`}
                  ...
                  <div className="flex justify-between items-end mb-3 text-sm">
                    <div>
                      <span className="text-[10px] font-bold text-gray-400 block mb-0.5 uppercase tracking-tighter">
                        ใช้ไปแล้ว
                      </span>
                    ...
                    <div className="text-right">
                      ...
                      <span
                        className={`font-black ${hasLimit ? "text-indigo-600" : "text-gray-300 italic"}`}
                      >
                        {hasLimit
                  ...
                  {isDanger && (
                    <div className="flex items-center gap-2 mt-4 text-red-500 text-[10px] font-black uppercase tracking-widest animate-pulse">
                      ...
                      <span>
                        {percent >= 100
                          ? "ถึงหรือเกินงบประมาณที่กำหนด"
                          : "ใกล้ถึงงบประมาณที่กำหนด"}
                      </span>
                    </div>
                  )}
                </div>
            ...
            <div className="mt-8 pt-6 border-t border-gray-100">
              <div className="flex justify-between items-center mb-4 px-2">
                <h3 className="font-bold text-sm text-gray-400 uppercase tracking-widest">
                  หมวดหมู่ที่ยังไม่ได้ตั้งงบ
                </h3>
                <span className="text-[10px] font-bold text-gray-300">
                  {unbudgetedBudgets.length} หมวดหมู่
                </span>
              </div>
              <div className="grid grid-cols-2 gap-3 pb-8">
                {unbudgetedBudgets.map((b) => (
                  <button
                    key={b._id}
                    onClick={() => handleEditLimit(b)}
                    className="flex items-center justify-between p-4 bg-gray-50/50 border border-gray-100 rounded-2xl hover:bg-surface hover:border-indigo-100 transition-all group"
                  >
            ...
            <div className="p-8">
              ...
              <div className="mb-10 text-center">
                <h3 className="text-2xl font-black mb-2">
                  {editingBudget.categoryId?.name}
                </h3>
                <p className="text-sm text-gray-400 font-medium">
                  ตั้งเป้างบประมาณที่คุณต้องการต่อเดือน
                </p>
              </div>

              <div className="space-y-6">
                <div className="relative">
                  <span className="absolute left-6 top-1/2 -translate-y-1/2 text-2xl font-black text-indigo-200">
                    ฿
                  </span>
                  <input
                    type="number"
